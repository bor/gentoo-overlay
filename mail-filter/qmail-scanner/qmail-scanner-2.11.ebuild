...
src_prepare() {
	...
	epatch "${FILESDIR}"/${PN}-2.11-posix-tolower-to-lc.patch # for perl > 5.26
	epatch "${FILESDIR}"/${PN}-2.11-disable-suid-check.patch  # bug 364123
...
src_configure() {
	...
	LANG=C PATH=${PATH}:/opt/f-prot:/opt/vlnx ./configure \
	--domain localhost \
	--batch \
	--log-details yes \
	--skip-setuid-test \
...
src_install () {
	...
	newins "${FILESDIR}"/${PN}.logrotate qmail-scanner

	exeinto /etc/cron.daily/
	newexe "${FILESDIR}"/${PN}.cronjob qmail-scanner
...
pkg_postinst () {
	...
	if use clamav && has_version "<app-antivirus/clamav-0.100"; then
